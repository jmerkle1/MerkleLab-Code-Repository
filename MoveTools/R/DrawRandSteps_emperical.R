...
#' @param simult When TRUE, the function will simultaneously draw a random step with a random turning angle. If FALSE, the two parameters will be drawn indepenedent of each other.
...
#'
#' @return   Returns a dataframe with each row representing a used or available step. The date column now corresponds to the date of the beginning of the step. The strata column provides the strata for the steps.
#'
#' @examples
#' # none
...
DrawRandSteps_emperical <- function(data=data,
                                    nr = 10,
                                    distMax = Inf,
                                    simult = FALSE,
                                    id_name="id",
  ...
  #manage packages
  if(all(c("circular","plyr","parallel") %in% utils::installed.packages()[,1])==FALSE)
